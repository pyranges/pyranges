...
initialized. To use `nb_cpu` with pyrle methods, you need to use `r.add(r2, nb_cpu=48)`, not
`r + r2`.
...
due to how strings are represented in memory in Python and Pandas. Use `%time`
in the IPython repl to test which is faster on your data and functions.