...
To iterate over multiple pyranges you can use pr.itergrs. It iterates over
multiple pyranges at the same time, returning the dfs belonging to the same
...
import pyranges as pr
cpg = pr.data.cpg()
cpg.Strand = "+" # ensure that cpg only has one strand
exons = pr.data.exons()

for cdf, edf in pr.itergrs([cpg, exons]):
    print("-----" * 5)
    print(cdf.head(1), edf.head(1))
```
