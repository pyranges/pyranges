...
There are 5 helper methods for printing PyRanges. These are p (regular print),
...
The print methods return the PyRanges object itself so that print methods can be used in pipes.

```{python tidy=FALSE}
import pyranges as pr

exons = pr.data.exons()
exons.p(n=2).mp(n=8).sp()
```
